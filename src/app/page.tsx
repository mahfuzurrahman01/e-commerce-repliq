        ...
        <div className="my-5 flex justify-center items-center flex-wrap text-center w-full gap-4">
          {products?.map((item: IProduct) => (
           ...
           <div className="flex space-x-4">
             <Image alt="" src={logo} className="object-cover w-12 h-12 rounded-full shadow bg-gray-500" />
             <div className="flex flex-col space-y-1">
               <a rel="noopener noreferrer" href="#" className="text-sm font-semibold">{item?.category}</a>
               <span className="text-xs text-gray-400">4 hours ago</span>
             </div>
           </div>
           <div>
             <img src={item?.image} alt="product" className="object-cover w-full mb-4 h-60 sm:h-96 bg-gray-500" />
             <h2 className="mb-1 text-xl font-semibold">{item?.title}</h2>
             <p className="text-sm text-gray-400">{item?.description.slice(0,100)}... <small className="text-red-900">read more</small></p>
           </div>
           <div className="flex flex-wrap justify-between">
             <div className="space-x-2">
               <h1 className="text-2xl text-red-900 font-bold">${item?.price}</h1>
             </div>
           ...
           <button onClick={() => addToCart(item)} className="py-2 bg-red-900 hover:bg-gray-300 hover:text-red-900 text-gray-300 duration-300">Add to cart</button>
         </div>
          ))}
        </div>
        <div className="flex justify-center mt-10 mb-20">
         <Link href='/products'>
         <button className="py-1 px-5 bg-red-900 hover:bg-gray-200 hover:text-red-900 text-gray-200 duration-300">See all</button>
         </Link>
        </div>
      </div>
    </div>
